...
By default the log files are located in the directory `PATH-OD-VLD/billing`. Under this directory the log files are organized in a tree data structure based on date (YYYY/MM). A separate file is generated for errors. The log file and the error file are tagged with the date.

log file: `PATH-OD-VLD/billing/2012/09/billing-2012.09.25`

error file: `PATH-OD-VLD/billing/2012/09/billing-error-2012.09.25`
...
A full explanation of the formatting is given in the `PATH-ODS-USD/defaults/billing.properties` file. For syntax questions please consult [StringTemplate v3 documentation] or the [cheat sheet].

On the web page generated by the HTTPD service (default port 2288), there is a link to `Action
      Log`. The table which appears there gives a summary overview extracted from the data contained in the billing log files.
...
1.  If the billing database does not already exist (see further below on migrating from an existing one), create it (we assume PSQL here):
        ...
        PROMPT-ROOT createdb -O dcache -U postgres billing

    If you are using a version of PSQL prior to 8.4, you will also need to do:

        PROMPT-ROOT createlang -U dcache plpgsql billing
...
2.  The property `billing.enable.db` controls whether the billing cell sends billing messages to the database. By default the option is disabled. To activate, set the value to `true` and restart the domain in which the HTTPD service is running.
    ...
    Extract from the layout file:
        ...
        PROMPT-ROOT dcache restart httpdDomain
